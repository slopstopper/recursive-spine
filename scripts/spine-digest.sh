...
# Assemble. Denominator always last. Titles/text are interpolated with %s
# only — never %b — so backslash sequences inside issue/milestone titles
# (e.g. "C:\config") cannot truncate or corrupt the rest of the digest.
printf '# Spine digest — %s\n\n' "$TODAY"
printf '%s' "$body"
printf '\n## Denominator\n\nswept %d/%d\n' "$swept" "$((swept+failed))"
[ -n "$fails" ] && printf '%s' "$fails"
